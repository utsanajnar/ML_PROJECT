...
def run(request):
    ...
    TRAINING_BASE = 'C:/Users/dheer/OneDrive/Desktop/Django/fileuse/fileuse/media/train'

    dirs = os.listdir('C:/Users/dheer/OneDrive/Desktop/Django/fileuse/fileuse/media/train')
    images = []
    names = []
    distinct_names = set()
    for label in dirs:
        if not label.startswith('.'):
            for i, fn in enumerate(os.listdir(os.path.join('media/train/', label))):
                print("")
                print(f"start collecting faces from {label}'s data")
                if not fn.startswith('.'):
                    print(f"analysing {fn}")
                    
                    cap = cv2.VideoCapture(os.path.join('media/train/', label, fn))
                    frame_count = 0
                    while True:
                        # read video frame
                        ...
                        if frame_count % 5 == 0 and raw_img is not None:
                            ...
                            if boxes.shape[0] > 0:
                                ...
                                # cv2.imwrite(f'/Volumes/SanketeSSD/FacerecogData/faces/tmp/{label}_{frame_count}.jpg', aligned_face)

                                aligned_face = aligned_face - 127.5
                                aligned_face = aligned_face * 0.0078125
                                images.append(aligned_face)
                                names.append(label)
                                distinct_names.add(label)
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            with open("imagefile/final_project/embeddings/embeddings.pkl", "wb") as f:
                pickle.dump((embeds, names, distinct_names), f)
            print("Done!")
